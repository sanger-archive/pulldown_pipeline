module Robots
  class Robot
    ...
    class Bed
      ...
      write_inheritable_attribute :attributes, [:api, :user_uuid, :purpose, :states, :label, :parent, :target_state, :parents, :robot]
    ...
    def beds=(new_beds)
      ...
      new_beds.sort_by {|id,bed| bed.order }.each do |id,bed|
        beds[id] = self.class::Bed.new(bed.merge({:api=>api, :user_uuid=>user_uuid, :robot=>self }))
      end
      @beds = beds
    end
    private :beds=
